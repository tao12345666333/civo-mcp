...
import { CREATE_INSTANCE_TOOL, LIST_INSTANCES_TOOL, REBOOT_INSTANCE_TOOL, SHUTDOWN_INSTANCE_TOOL, START_INSTANCE_TOOL, RESIZE_INSTANCE_TOOL, DELETE_INSTANCE_TOOL } from "./tools/instances.js";
import { GET_DISK_IMAGE_TOOL, LIST_DISK_IMAGES_TOOL } from "./tools/disk-images.js";
import { LIST_SIZES_TOOL } from "./tools/sizes.js";
import { LIST_REGIONS_TOOL } from "./tools/regions.js";
import { LIST_NETWORKS_TOOL, CREATE_NETWORK_TOOL, RENAME_NETWORK_TOOL, DELETE_NETWORK_TOOL } from "./tools/networks.js";
...
import { LIST_KUBERNETES_CLUSTERS_TOOL, CREATE_KUBERNETES_CLUSTER_TOOL, DELETE_KUBERNETES_CLUSTER_TOOL, LIST_KUBERNETES_VERSIONS_TOOL } from "./tools/kubernetes.js";
  ...
  try {
    ...
    switch (name) {
      ...
      case "list_instances": {
        ...
        const instanceList = instances.items.map(i => 
        ...
        case "list_disk_images": {
          ...
          const imageList = images.items.map(i => 
      ...
      case "reboot_instance": {
        ...
        const result = await rebootInstance(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "shutdown_instance": {
        ...
        const result = await shutdownInstance(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "start_instance": {
        ...
        const result = await startInstance(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "resize_instance": {
        ...
        const result = await resizeInstance(args as { id: string; size: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "list_regions": {
        ...
        const regionList = regions.map((r: any) => 
          `${r.name} (${r.code})`
        ).join("\n");
      ...
      case "list_networks": {
        ...
        const networkList = networks.map((n: any) => 
          `${n.name} (${n.id}) - ${n.label}`
        ).join("\n");
      ...
      case "create_network": {
        ...
        const network = await createNetwork(args as { label: string; region?: string });
        return {
          content: [{
            type: "text",
      ...
      case "rename_network": {
        ...
        const network = await renameNetwork(args as { id: string; label: string; region?: string });
        return {
          content: [{
            type: "text",
      ...
      case "delete_network": {
        ...
        const result = await deleteNetwork(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "delete_instance": {
        ...
        const result = await deleteInstance(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
      ...
      case "list_kubernetes_clusters": {
        ...
        const clusterList = clusters.items.map((c: any) => 
      ...
      case "create_kubernetes_cluster": {
        ...
        const cluster = await createCluster(args as { name: string; region: string; network_id: string; nodes: number; node_size: string; kubernetes_version: string; });
        return {
          content: [{
            type: "text",
      ...
      case "delete_kubernetes_cluster": {
        ...
        const result = await deleteCluster(args as { id: string; region: string });
        return {
          content: [{
            type: "text",
